...
with DAG('weather_pull_leeds_dag', start_date=datetime(2022, 1, 1), schedule_interval='@daily', catchup=False) as dag:

    create_db = MySqlOperator(
        task_id = 'create_db',
        sql = 'CREATE database IF NOT EXISTS weather; USE weather; CREATE TABLE IF NOT EXISTS leeds (Timestamp Datetime PRIMARY KEY, City VARCHAR(255), Population INT, Country VARCHAR(10),Latitude FLOAT, Longitude FLOAT,Temperature FLOAT,Temp_min FLOAT,Temp_max FLOAT,Pressure INT, Humidity INT, Visibility INT, Wind_speed FLOAT,Wind_degree FLOAT, Rain_1h FLOAT,Rain_3h FLOAT, Snow_1h FLOAT,Snow_3h FLOAT, Weather_ID INT, Weather_group VARCHAR(255), Description VARCHAR(255), Cloudiness INT, Sunrise Datetime, Sunset Datetime, Air_quality_index INT, CO FLOAT, NO FLOAT, NO2 FLOAT, O3 FLOAT, SO2 FLOAT, NH3 FLOAT, PM2_5 FLOAT, PM10 FLOAT);',
        mysql_conn_id = 'mysqldb'
    )
    ...
    drop_table = MySqlOperator(
        task_id = 'drop_table',
        sql = 'USE weather; DROP TABLE IF EXISTS leeds;',
        mysql_conn_id = 'mysqldb'
    ) 
